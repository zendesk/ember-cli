...
module.exports = class AddonDiscovery {
  ...
  */
  ...
  discoverFromDependencies(root, pkg, excludeDevDeps) {
    let addons = Object.keys(this.dependencies(pkg, excludeDevDeps))
      .map((name) => {
        if (name !== 'ember-cli') {
  ...
  discoverInRepoAddons(root, pkg) {
    ...
    return pkg['ember-addon'].paths.reduce((sum, addonPath) => {
  ...
  discoverFromInternalProjectAddons(project) {
    return project.supportedInternalAddonPaths().reduce((sum, path) => {
