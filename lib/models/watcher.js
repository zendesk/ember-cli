...
Watcher.detectWatcher = function(ui, _options) {
  ...
  if (options.watcher === 'polling') {
    ...
    return exec('watchman version').then(function(output) {
      ...
      // watchman has adopted a date formatted versioning system and as such,
      // are no longer semver valid. Perform a check and default to satisfying
      // requirements if that is the case.
      var minVersionSatisfied = semver.valid(version) ? semver.satisfies(version, '>= 3.0.0') : true;
      var canNestRootsSatisfied = semver.valid(version) ? semver.satisfies(version, '>= 3.7.0') : true;
      ...
      if (minVersionSatisfied) {
        ...
        options._watchmanInfo = {
          enabled: true,
          version: version,
          canNestRoots: canNestRootsSatisfied
        };
      } else {
